...
| `dagster_tests/` | A Python directory that contains tests for `dagster`                      |
...
pyenv virtualenv $PYTHON_VERSION dagster
...
```bash
export PYTHON_VERSION=X.Y.Z
conda create --name dagster python=PYTHON_VERSION
...
Tests can be found in `dagster_tests` and are run with the following command:

```bash
pytest dagster_tests
```

As you create Dagster ops and graphs, add tests in `dagster_tests/` to check that your
